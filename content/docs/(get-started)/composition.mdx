---
title: Composition
description: A guide to composing Lumi UI components with your own React components and elements.
---

## A Flexible Foundation

In Lumi UI, composition is powered by a single, flexible concept: the **`render` prop**. This approach, inherited from Base UI, provides a clear and explicit way to customize the underlying structure of a component without sacrificing accessibility or functionality.
...
Every Lumi UI component renders the most appropriate element by default (e.g., a `<button>` for a trigger, a `<div>` for a menu item). However, you might need a `Menu.Item` to act as a link. The `render` prop makes this trivial.

```tsx title="components/MyNavigation.tsx"
...
If you're used to Radix UI, you're likely familiar with the `asChild` prop. The `render` prop in Lumi UI serves the same fundamental purpose — composing components — but with a slightly different and more explicit mental model.

| Concept | Radix UI (`asChild`) | Lumi UI (`render`) |
| :--- | :--- | :--- |
...
The most powerful use case for `render` is composing Lumi UI primitives with your own custom React components. For example, you can use a custom, styled `NextLink` component as a `Menu.Trigger`.

```tsx title="components/MyComponent.tsx"
// Assume NextLink is a custom component that wraps next/link
import { NextLink } from '@/lib/components/NextLink';
import { Menu } from '@/lib/components/Menu';
...
When you do this, your custom component **must honor a contract**: it needs to forward the `ref` and spread all props it receives onto the underlying DOM node. This ensures that accessibility attributes and event handlers from the Base UI primitive are correctly applied.

```tsx title="components/NextLink.tsx" {5}
import Link from 'next/link';
import * as React from 'react';

const NextLink = React.forwardRef<HTMLAnchorElement, MyLinkProps>(
  (props, ref) => {
...
```tsx title="components/MySwitch.tsx"
...
This pattern gives you direct access to the component's machinery, allowing for powerful and dynamic compositions while staying true to Lumi UI's philosophy of keeping you close to the metal.
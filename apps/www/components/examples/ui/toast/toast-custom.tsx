...
export function ToastCustomDemo() {
  ...
  const updateToast = (id: string) => {
    toast.update(id, {
      customContent: <NoWorriesToast />,
    });
    setToastId((currentToastId) =>
      currentToastId === id ? null : currentToastId,
    );
  };
  ...
  const createToast = () => {
    if (toastId) return;
    const id = toast.add({
      customContent: <AchievementToast onNoThanks={() => updateToast(id)} />,
      onRemove: () => {
        setToastId((currentToastId) =>
          currentToastId === id ? null : currentToastId,
        );
...
function AchievementToast({ onNoThanks }: { onNoThanks: () => void }) {
  return (
      ...
      <div className="relative flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="mt-2 flex justify-between items-center">
            ...
            <Button onClick={onNoThanks} size="sm">
              No, thanks!
            </Button>
          </div>
        </div>

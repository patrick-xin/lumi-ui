...
`CommandInput` is built on `AutocompleteInputGroupContent` and can show a clear button (`showClear`).
...
A composite input based on `AutocompleteInputGroupContent`.
...
| `...props` | `React.ComponentProps<typeof AutocompleteInputGroupContent>` | Passes all other input-group props. |

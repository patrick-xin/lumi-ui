---
title: Dialog
description: A popup that opens on top of the entire page.
links:
  doc: https://base-ui.com/react/components/dialog
  api: https://base-ui.com/react/components/dialog#api-reference
---

<ComponentPreview name="dialog-demo" />

## Installation

<CodeTabs>
  <TabsList>
    <TabsTab value="cli">CLI</TabsTab>
    <TabsTab value="manual">Manual</TabsTab>
  </TabsList>

  <TabsPanel value="cli">
    ```bash
    npx shadcn@latest add @lumi-ui/dialog
    ```
  </TabsPanel>

  <TabsPanel value="manual">
    <Steps>
      <Step title="Copy and paste the following code into your project.">
        <ComponentSourceCode name="dialog" title="dialog.tsx" collapsible/>
      </Step>
    </Steps>
  </TabsPanel>
</CodeTabs>

Add following utilities to your `globals.css`:

```css
@utility animate-fade-up {
  @apply transition-all duration-200 ease-[cubic-bezier(0.16,1,0.3,1)];

  &[data-starting-style],
  &[data-ending-style] {
    opacity: 0;
    scale: 0.96;
    translate: 0 0.75rem;
  }
}

@utility animate-fade-down {
  @apply transition-all duration-200 ease-[cubic-bezier(0.16,1,0.3,1)];

  &[data-starting-style],
  &[data-ending-style] {
    opacity: 0;
    scale: 0.96;
    translate: 0 -0.75rem;
  }
}

@utility animate-fade-zoom {
  @apply transition-all duration-200 ease-[cubic-bezier(0.16,1,0.3,1)];

  &[data-starting-style],
  &[data-ending-style] {
    opacity: 0;
    scale: 0.96;
  }
}

@utility animate-fade {
  @apply transition-all duration-100;

  &[data-starting-style],
  &[data-ending-style] {
    opacity: 0;
  }
}
```

## Basic Usage
```tsx
import {
  Dialog,
  DialogClose,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogContent,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";

export function DialogDemo() {
  return (
    <Dialog>
      <DialogTrigger>Show Alert Dialog</DialogTrigger>
      <DialogContent showCloseButton>
        <DialogHeader>
          <DialogTitle>Are you absolutely sure?</DialogTitle>
          <DialogDescription>
            This action cannot be undone. This will permanently delete your
            account and remove your data from our servers.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter>
          <DialogClose>Cancel</DialogClose>
          <DialogClose>Continue</DialogClose>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
```

## Anatomy

<Tabs defaultValue="common">
  <TabsList>
    <TabsTab value="common">Common usage</TabsTab>
    <TabsTab value="primitive">Primitive usage</TabsTab>
  </TabsList>

  <TabsPanel value="common">
```tsx
<Dialog>   
  <DialogTrigger />
  <DialogContent>      
    <DialogHeader>     
      <DialogTitle />     
      <DialogDescription />     
    </DialogHeader>
    <DialogScrollArea />     
    <DialogFooter>     
      <DialogClose />     
    </DialogFooter>   
  </DialogContent> 
</Dialog>
```
  </TabsPanel>
  <TabsPanel value="primitive">
```tsx
<Dialog>
  <DialogTrigger />
  <DialogPortal>
    <DialogBackdrop />
    <DialogViewport>
      <DialogPopup>
        <DialogHeader>
          <DialogTitle />
          <DialogDescription />
        </DialogHeader>
        <DialogFooter>
          <DialogClose />
        </DialogFooter>
      </DialogPopup>
    </DialogViewport>
  </DialogPortal>
</Dialog>
```
  </TabsPanel>
</Tabs>

## Variants

<ComponentPreview name="dialog-variants" />

## Cookbook

### Scrolling

#### Inside scroll
Set `DialogContent` variant to `layout="scrollable"`, use `ScrollArea` as the scrolling container or add `overflow-y-auto` to the content. Adding any content above or below scrollable content will make them fixed.

<ComponentPreview name="dialog-inside-scroll" />

#### Placing elements outside the popup
Set `DialogContent` variant to `element-outside`. Adjust `padding` in `viewportVariants` fit your design needs.

<ComponentPreview name="dialog-elements-outside" />

#### Outside scroll
To allow the entire dialog—including the header and footer—to scroll with the page, use `DialogViewport` as the scrolling container. This approach is useful for very long content where internal scrolling is not desired.

<ComponentPreview name="dialog-outside-scroll" />

### Close confirmation
Use `<DialogContent layout="stacked"/>` and [`<AlertDialogContent layout="stacked" />`](/docs/components/overlays-dialogs/alert-dialog#composite-component). Nest `<AlertDialogContent />` inside `<Dialog />`.

<ComponentPreview name="dialog-close-confirmation" />


### Controlled dialog
By default, Dialog is an **uncontrolled** component that manages its own state. Use the `open` and `onOpenChange` props to control the dialog state. This is useful when triggering the dialog from external events or performing async actions before closing.

```tsx title="Controlled dialog"
const [open, setOpen] = React.useState(false);

<Dialog open={open} onOpenChange={setOpen}>
  <DialogTrigger>Open</DialogTrigger>
  <DialogContent>
    <form 
      // Close the dialog once the form data is submitted           
      onSubmit={
        async () => {             
          await submitData();             
          setOpen(false);   
        }
      }
    >
      ...
    </form>
  </DialogContent>
</Dialog>
```
It’s also common to use `onOpenChange` if your app needs to do something when the dialog is closed or opened. This is recommended over `useEffect` when reacting to state changes.

```tsx title="Running code when dialog state changes"
<Dialog
  open={open}
  onOpenChange={(open) => {
    // Do stuff when the dialog is closed
    if (!open) {
      doStuff();
    }
    // Set the new state
    setOpen(open);
  }}
>
```

#### Open from Dropdown menu
To open a dialog from a [`DropdownMenu`](/docs/components/overlays-dialogs/dropdown-menu), use the controlled state pattern. The Dialog should be placed outside the `DropdownMenu` to avoid nesting issues.

<ComponentPreview name="dialog-from-menu" />



### Detached Triggers
If you need to trigger a dialog from a completely different part of the component tree, or if nesting the trigger inside the root is impractical, use `createDialogHandle` to link a `DialogTrigger` to a `Dialog` remotely.

```tsx title="Detached trigger"
import { createDialogHandle } from "@/components/ui/dialog"

const demoDialog = createDialogHandle();

<DialogTrigger handle={demoDialog}>Open</DialogTrigger>

<Dialog handle={demoDialog}>
  ...
</Dialog>
```
<ComponentPreview name="dialog-detached-trigger" />

### Multiple triggers & payloads
A single dialog can serve multiple triggers. You can pass a payload from the trigger to the dialog to render dynamic content based on which element opened it. This is highly effective for list views where every item needs an "Edit" or "Delete" modal without rendering a separate dialog instance for each row.

```tsx title="Multiple triggers within the Root part"
<Dialog>
  <DialogTrigger>Trigger 1</DialogTrigger>
  <DialogTrigger>Trigger 2</DialogTrigger>
  ...
</Dialog>
```
```tsx title="multiple detached triggers"
const demoDialog = createDialogHandle();

<DialogTrigger handle={demoDialog}>Trigger 1</DialogTrigger>
<DialogTrigger handle={demoDialog}>Trigger 2</DialogTrigger>
<Dialog handle={demoDialog}>
  ...
</Dialog>
```

The dialog can render different content depending on which trigger opened it. This is achieved by passing a payload to the `<DialogTrigger>` and using the `function-as-a-child` pattern in `<Dialog>`.

<ComponentPreview name="dialog-detached-triggers-with-payload" />

### Controlled mode with multiple triggers
When controlling a multi-trigger dialog externally, you must manage both the open state and the triggerId. The `onOpenChange` event provides `eventDetails` containing the ID of the trigger that initiated the action.

<ComponentPreview name="dialog-controlled-mode-with-multiple-triggers" />

## API reference

### Primitives

| Component | Description |
| :--- | :--- |
| [`Dialog`](https://base-ui.com/react/components/dialog#root) | Groups all parts of the dialog. Doesn’t render its own HTML element. |
| [`DialogTrigger`](https://base-ui.com/react/components/dialog#trigger) | A button that opens the dialog. |
| [`DialogPortal`](https://base-ui.com/react/components/dialog#portal) | A portal element that moves the popup to a different part of the DOM. Renders a `<div>` element.|
| [`DialogBackdrop`](https://base-ui.com/react/components/dialog#backdrop) | An overlay displayed beneath the popup. Renders a `<div>` element. |
| [`DialogViewport`](https://base-ui.com/react/components/dialog#viewport) | A positioning container for the dialog popup that can be made scrollable. Renders a `<div>` element. |
| [`DialogPopup`](https://base-ui.com/react/components/dialog#popup) | A container for the dialog contents. Must be used inside `DialogPortal`. Renders a `<div>` element. |
| `DialogHeader` | Styled container for convenience purpose. Renders a `<div>` element. |
| [`DialogTitle`](https://base-ui.com/react/components/dialog#title) | A heading that labels the dialog. Renders an `<h2>` element. |
| [`DialogDescription`](https://base-ui.com/react/components/dialog#description) | A paragraph with additional information about the dialog. Renders a `<p>` element. |
| `DialogFooter` | Styled container for convenience purpose. Renders a `<div>` element. |
| [`DialogClose`](https://base-ui.com/react/components/dialog#close) | A button that closes the dialog. Renders a `<button>` element.|

### Composite components

| Component | Description |
| :--- | :--- |
| `DialogContent` | **For common usage**. Wraps Portal, Backdrop, Viewport, and Popup with standard centered styling. |
| `DialogElementOutsideContent` | Wraps Portal, Viewport, Popup and a `div` with `pointer-events-auto` class to handle dialog interactions. |
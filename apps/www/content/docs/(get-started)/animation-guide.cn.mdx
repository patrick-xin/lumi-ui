...
| `animate-dialog` | 用于模态框和对话框。 |
| `animate-backdrop` | 用于背景遮罩层。 |
...
移除 `animate-popup`、`animate-dialog` 或 `animate-backdrop` 工具类，并使用标准的 Tailwind 类来获得完全的控制权：
...
使用 `animate-dialog` 和 `animate-backdrop` 的组件：[对话框](/docs/components/overlays-dialogs/dialog) | [警示对话框](/docs/components/overlays-dialogs/alert-dialog)
...
```tsx
// AI-friendly
import { ComboboxInputGroupContent, ComboboxItem, ComboboxPortal } from '@/components/ui/combobox'

// Nested namespaces - AI struggles
import { Combobox } from '@base-ui/react/combobox'
// AI must now infer: Combobox.Input? Combobox.Item? ComboboxInputGroupContent?
```
...
### Primitives + Consistent Styles

When you drop down to the primitive layer for maximum layout control, you aren't using the pre-assembled composites. However, you don't lose the global design language. Because Lumi UI leverages built-in global CSS utilities, you can build entirely custom structures while retaining the exact same hover effects, animations, and container treatments as the rest of the application.

For example, here's a fully custom `<Select>` built with primitives. It utilizes `overlay-outline`, `animate-popup`, and `highlight-on-active` to maintain perfect visual consistency:
